...
ma::Model* global_audio_model = nullptr;
bool audio_initialized = false;
int audio_counter = 1;

static void registerAudioProcessor(HttpService& router) {
    router.GET("/audioprocessor", [](HttpRequest* req, HttpResponse* resp) {
        hv::Json res;
        res["Code"] = 1;
        res["Msg"]  = "";
        
        // Get duration from query parameter, default to 3 seconds
        int duration = 3;
        if (req->query_params.find("duration") != req->query_params.end()) {
            duration = std::stoi(req->query_params["duration"]);
            if (duration <= 0 || duration > 30) {  // Limit to reasonable range
                duration = 3;
            }
        }
        ...
        // Capture audio with specified duration
        auto audio_data = capture_audio(duration);
        
        res["Data"] = "Audio processing completed (model loading disabled for testing)";
        res["samples"] = audio_data.size();
        res["duration_seconds"] = duration;
        res["sample_rate"] = 16000;
    ...
    router.GET("/audiotest", [](HttpRequest* req, HttpResponse* resp) {
        hv::Json res;
        res["Code"] = 1;
        res["Msg"]  = "";
        ...
        auto start = std::chrono::steady_clock::now();
        MA_LOGI("http", "Calling capture_audio with duration = %d", duration);
        auto audio_data = capture_audio(duration);
        MA_LOGI("http", "capture_audio returned %zu samples", audio_data.size());
        res["Data"] = "Audio captured successfully";
        res["samples"] = audio_data.size();
        res["duration_requested"] = duration;
        res["Duration"] = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start).count();
        res["Timestamp"] = getTimestamp();
